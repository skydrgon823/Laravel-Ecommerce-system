@if (get_payment_setting('status', SSLCOMMERZ_PAYMENT_METHOD_NAME) == 1)
    <li class="list-group-item">
        <input class="magic-radio js_payment_method" type="radio" name="payment_method" id="payment_{{ SSLCOMMERZ_PAYMENT_METHOD_NAME }}"
               ...
               @if ((session('selected_payment_method') ?: setting('default_payment_method')) == SSLCOMMERZ_PAYMENT_METHOD_NAME) checked @endif
        >
        <label for="payment_{{ SSLCOMMERZ_PAYMENT_METHOD_NAME }}">{{ get_payment_setting('name', SSLCOMMERZ_PAYMENT_METHOD_NAME) }}</label>
        <div class="payment_{{ SSLCOMMERZ_PAYMENT_METHOD_NAME }}_wrap payment_collapse_wrap collapse @if ((session('selected_payment_method') ?: setting('default_payment_method')) == SSLCOMMERZ_PAYMENT_METHOD_NAME) show @endif">
